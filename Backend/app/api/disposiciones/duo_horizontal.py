def handle_region(region) -> str:
    ...
    html_content = f"<div style='width: 100%; display: flex; flex-direction: column; justify-content: center; {align_style}'{data_tts_html}>\n"
...
{codigo_content}
            ...
            for fila in tabla.findall("fila"):
                ...
                for celda in fila.findall("celda"):
                    ...
                    html_content += f"        <td style='text-align: center; font-size: 0.9em;'>{celda_content}</td>\n"
                html_content += "    </tr>\n"
            ...
            if descripcion:
                html_content += f"""
                <div style="margin:0; text-align: center; font-style: italic; font-size: 0.9em;">
            ...
            if url:
                html_content += f"""
                <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; max-width: 100%; max-height: 100%;"{data_tts_html}>
                    <img src="{url}" alt="{descripcion}" 
                         style="max-width: 100%; max-height: 18vh; height: auto; object-fit: contain;">
                """
                if descripcion:
                    html_content += f"""
...
def handle_duo_horizontal_diapositiva(diapositiva) -> str:
    """
    Maneja el contenido de una diapositiva con disposición 'duo-horizontal',
    dividiendo el contenido en dos regiones: arriba y abajo.
    Preserva el orden de los elementos según el XML.
    """
    ...
    # Contenedor para las dos regiones, una arriba y otra abajo
    # Agregamos gap para que haya espacio entre la región de arriba y la de abajo.
    html_content += f"<div style='flex: 1; display: flex; flex-direction: column; height: 90%;'>\n"
    
    # Obtener las regiones
    regiones = diapositiva.findall(".//region")
    if len(regiones) != 2:
        raise ValueError("La disposición 'duo-horizontal' requiere exactamente dos regiones.")
    
    # Procesar cada región y asignar a arriba o abajo
    for index, region in enumerate(regiones):
        posicion = "arriba" if index == 0 else "abajo"
        # Puedes utilizar 'posicion' para aplicar estilos específicos si lo deseas
        html_content += (
            "<div style='flex: 1; "
            "display: flex; "
