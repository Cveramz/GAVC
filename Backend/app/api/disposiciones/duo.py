...
def handle_duo_diapositiva(diapositiva) -> str:
    """
    Maneja el contenido de una diapositiva con disposición 'duo',
    dividiendo el contenido en dos regiones: izquierda y derecha.
    Preserva el orden de los elementos según el XML.
    """
    ...
    # Contenedor para las dos regiones
    html_content += f"<div style='flex: 1; display: flex; height: 90%;'>\n"
    
    # Obtener las regiones
    regiones = diapositiva.findall(".//region")
    if len(regiones) != 2:
        raise ValueError("La disposición 'duo' requiere exactamente dos regiones.")
    
    # Procesar cada región y asignar a izquierda o derecha
    for index, region in enumerate(regiones):
        posicion = "izquierda" if index == 0 else "derecha"
